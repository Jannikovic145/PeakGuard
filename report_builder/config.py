...
class PeakGuardColors:
    ...
    Abgestimmt auf das Web-Dashboard Design
    """

    # ReportLab Colors - Abgestimmt auf Website
    PRIMARY = colors.HexColor("#1E3A5F")      # PeakGuard Dunkelblau (Website)
    SECONDARY = colors.HexColor("#E67E22")    # Orange (Website)
    ACCENT = colors.HexColor("#3498DB")       # Hellblau (Gradient End)
    SUCCESS = colors.HexColor("#27AE60")      # Erfolg Grün (Website)
    WARNING = colors.HexColor("#F39C12")      # Warnung Orange
    DANGER = colors.HexColor("#E74C3C")       # Kritisch Rot (Website)

    DARK = colors.HexColor("#2C3E50")         # Website Dark
    GRAY_DARK = colors.HexColor("#34495E")
    GRAY = colors.HexColor("#7F8C8D")
    GRAY_LIGHT = colors.HexColor("#ECF0F1")   # Website Light
    GRAY_LIGHTER = colors.HexColor("#F8F9FA")
    WHITE = colors.white

    # Matplotlib Colors (hex strings) - Abgestimmt auf Website
    MPL = {
        'primary': '#1E3A5F',
        'primary_dark': '#1E3A5F',
        'secondary': '#E67E22',
    ...
    def setup_matplotlib_theme(cls) -> None:
        """Konfiguriert Matplotlib mit PeakGuard-Design - Hochauflösend"""
        plt.rcParams.update({
            # Hintergrund
            ...
            'figure.dpi': 100,  # Bildschirm-DPI
            'savefig.dpi': 300,  # Speicher-DPI (Druckqualität)
            'savefig.format': 'png',
            'savefig.bbox': 'tight',
            'savefig.pad_inches': 0.1,

            # Font-Rendering für schärfere Schriften
            # DejaVu Sans zuerst für griechische Zeichen (cosϕ)
...
class ReportConfig:
    ...
    # Chart-Einstellungen - Hochauflösend für PDF
    chart_dpi: int = 300  # Druckqualität (300 DPI)
